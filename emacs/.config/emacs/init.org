...
- [[#appearance][Appearance]]
  ...
  - [[#modeline][ModeLine]]
  - [[#rainbow-mode-disabled][Rainbow Mode (Disabled)]]
  - [[#theme][Theme]]
...
- [[#cuamode-disabled][CuaMode (Disabled)]]
...
- [[#completion][Completion]]
  ...
  - [[#orderless][Orderless]]
  - [[#savehist][Savehist]]
  - [[#vertico-and-marginalia-disabled][Vertico and Marginalia (Disabled)]]
  - [[#embark][Embark]]
  - [[#jinx][Jinx]]
  - [[#ivy--counsel-enabled][Ivy + Counsel (Enabled)]]
- [[#dashboard][Dashboard]]
- [[#dired][Dired]]
  - [[#hacks][Hacks]]
  - [[#previews][Previews]]
...
#+begin_src emacs-lisp
(use-package nerd-icons-ivy-rich :straight (:host github :repo "seagle0128/nerd-icons-ivy-rich"))
(use-package beacon :straight (:host github :repo "Malabarba/beacon"))
(use-package whitespace4r :straight (:host github :repo "twlz0ne/whitespace4r.el"))
(use-package project)
(use-package flymake)
#+end_src
...
#+begin_src emacs-lisp
(use-package evil
  :custom      ;; tweak evil's configuration before loading it
...
(advice-add 'evil-yank :around #'b2f/pulse-on-yank)

;; For selecting text and seach backw/forw with # and *
...
(use-package general
  ...
  (leader-keys
    ...
    "b x" '(b2f/kill-current-buffer :wk "Kill this buffer")
    "b o" '(b2f/kill-other-buffers  :wk "Kill other buffers")
  ...
  (leader-keys
    "d"       '(:ignore t		     :wk "Dired...")
    "d j"     '(dired			     :wk "Dired Jump (ask)")
    "d d"     '(b2f/dired-jump		     :wk "Open Dired")
    "d v"     '(b2f/dired-jump-v	     :wk "Open Dired (V)")
    "d s"     '(b2f/dired-jump-h	     :wk "Open Dired (H)")
    "d a"     '(b2f/dired-create-file-or-dir :wk "Create File or Dir")
  ...
  (leader-keys
    "c"   '(:ignore t	      :wk "Cursors...")
    "c c" '(b2f/mc-place-here :wk "Add cursor (paused)")
    "c u" '(b2f/mc-unpause    :wk "Upause cursors")
    "c p" '(b2f/mc-pause      :wk "Pause cursors"))
  ...
  (leader-keys
    ...
    "l n" '(flymake-goto-next-error :wk "Flymake next error")
    "l p" '(flymake-goto-prev-error :wk "Flymake previous error")
  ...
  (leader-keys
    ...
    "s c" '(start/open-init-file			:wk "Open config")
...
(use-package doom-modeline
  ...
  :custom
  (doom-modeline-height 31)      ;; sets modeline height
  (doom-modeline-bar-width 4)    ;; sets right bar width
  ;; (doom-modeline-persp-name t)   ;; adds perspective name to modeline
  ;; (doom-modeline-persp-con t)   ;; adds folder icon next to persp name
  (inhibit-compacting-font-caches t) ;; don’t compact font caches during GC - for performance
  :config
...
(defun b2f/mood-line-evil-state ()
  "Return full Evil state name for mood-line."
  (when (boundp 'evil-state)
    (pcase evil-state
      ('normal   (propertize "[NORMAL]"   'face '(:foreground "#eb6f92" :weight bold)))
      ('insert   (propertize "[INSERT]"   'face '(:foreground "#9ccfd8" :weight bold)))
      ('visual   (propertize "[VISUAL]"   'face '(:foreground "#f6c177" :weight bold)))
      ('replace  (propertize "[REPLACE]"  'face '(:foreground "#eb6f92" :weight bold)))
      ('operator (propertize "[OPERATOR]" 'face '(:foreground "#c4a7e7" :weight bold)))
      ('motion   (propertize "[MOTION]"   'face '(:foreground "#c4a7e7" :weight bold)))
      ('emacs    (propertize "[EMACS]"    'face '(:foreground "#f6c177" :weight bold)))
      (_ ""))))

(advice-add 'mood-line-segment-modal :override #'b2f/mood-line-evil-state)
#+end_src
...
(custom-set-faces
 '(org-link ((t (:inherit org-link :underline t)))))

;; (use-package almost-mono-themes
;;   :straight (:host codeberg :repo "cryon/almost-mono-themes")
;;   :config
...
#+begin_src emacs-lisp :tangle no
(use-package snippy
  :straight (:host github :repo "MiniApollo/snippy.git"
            :branch "main"
            :rev :newest)
...
(use-package cmake-ts-mode :straight nil :mode ("CMakeLists\\.txt\\'" "\\.cmake\\'"))
(use-package go-mod-ts-mode :straight nil :mode "\\.mod\\'")
(use-package lua-ts-mode :straight nil :mode "\\.lua\\'")
(use-package rust-ts-mode :straight nil :mode "\\.rs\\'")
(use-package typescript-ts-mode :straight nil :mode "\\.ts\\'")
(use-package tsx-ts-mode :straight nil :mode "\\.tsx\\'")
(use-package yaml-ts-mode :straight nil :mode ("\\.yaml\\'" "\\.yml\\'"))
#+end_src
...
#+begin_src emacs-lisp
;(savehist-mode 1) ;; Enables save history mode
#+end_src

** Vertico and Marginalia (Disabled)
...
(use-package dashboard
  ...
  (dashboard-vertically-center-content nil) ;; set to 't' for centered content vertically
...
(use-package dired
  :straight nil
  :custom
  (dired-listing-switches "-avlD --group-directories-first --human-readable")
  (dired-omit-files "\\|^\\..+$")
  (dired-dwim-target t)
...
(with-eval-after-load 'dired
  (evil-define-key 'normal dired-mode-map
    (kbd "l") 'b2f/dired-enter-directory
    (kbd "h") 'b2f/dired-up-directory
    (kbd "a") 'b2f/dired-create-file-or-dir))

;; (with-eval-after-load 'dired
;;   (global-set-key dired-mode-map [mouse-1] 'b2f/dired-up-directory))
#+end_src
...
(use-package dired-narrow
    ...
    :straight (:host github
                ...
                :files ("dired-narrow.el" "dired-ranger.el"))
    :config
    (evil-define-key 'normal dired-mode-map
        (kbd "v") 'dired-mark
...
gzm — make cursors for all matches
gzz — add cursor at point
gzn / gzp — next/prev match
...
(use-package evil-mc
  ...
  (evil-define-key '(normal visual) 'global
    ;; C   — add cursor on next match (helix-style, repeat to keep adding)
    (kbd "C")         'evil-mc-make-and-goto-next-match
    ;; M-C — add cursor on prev match
    (kbd "M-C")       'evil-mc-make-and-goto-prev-match
    ;; skip without adding cursor
    (kbd "M-n")       'evil-mc-skip-and-goto-next-match
    (kbd "M-p")       'evil-mc-skip-and-goto-prev-match
    ;; , — collapse all cursors (helix-style)
...
;; Full line-by-line scrolling configuration
(setq scroll-step 1)                  ; Line-by-line when cursor exits window
(setq scroll-conservatively 100)      ; Avoid recentering
(setq scroll-margin 5)
(setq scroll-preserve-screen-position t) ; Keep cursor visual position
